...
//
static SetNode node_create(Pointer key, Pointer value) {
...
static SetNode node_find_equal(SetNode node, CompareFunc compare, Pointer key) {
	...
	//
	int compare_res = compare(key, node->key);			// αποθήκευση για να μην καλέσουμε την compare 2 φορές
	if (compare_res == 0)									// value ισοδύναμη της node->value, βρήκαμε τον κόμβο
		...
		return node_find_equal(node->left, compare, key);
	else													// value > node->value, ο κόμβος που ψάχνουμε είνια στο δεξιό υποδέντρο
		return node_find_equal(node->right, compare, key);
}
...
static SetNode node_find_previous(SetNode node, CompareFunc compare, SetNode target) {
	...
	} else if (compare(target->key, node->key) < 0) {  // παντου αντι για value εχω key
...
static SetNode node_insert(SetNode node, CompareFunc compare, Pointer key, Pointer value, bool* inserted, Pointer* old_key, Pointer* old_value) {
	...
	int compare_res = compare(key, node->key);
	if (compare_res == 0) {
		...
		*old_key = node->key;
		...
		node->left = node_insert(node->left, compare, key, value, inserted, old_key, old_value);

	} else {
		// value > node->value, συνεχίζουμε δεξιά
		node->right = node_insert(node->right, compare, key, value, inserted, old_key, old_value);
	}
...
static SetNode node_remove(SetNode node, CompareFunc compare, Pointer key, Pointer value, bool* removed, Pointer* old_key, Pointer* old_value) {
	if (node == NULL) {
	...
	if (compare_res < 0)
		node->left  = node_remove(node->left,  compare, key, value, removed, old_key, old_value);
	else
		node->right = node_remove(node->right, compare, key, value, removed, old_key, old_value);
...
static void node_destroy(SetNode node, DestroyFunc destroy_key, DestroyFunc destroy_value) {
	if (node == NULL)
		return;
...
Set set_create(CompareFunc compare, DestroyFunc destroy_key, DestroyFunc destroy_value) {
...
void set_insert(Set set, Pointer key, Pointer value) {
	bool inserted;
	Pointer old_key;
	Pointer old_value;
	set->root = node_insert(set->root, set->compare, key, value, &inserted, &old_key, &old_value);
	...
	if (inserted)
		...
		if (set->destroy_key!=NULL) set->destroy_key(old_key);
	}
}

bool set_remove(Set set, Pointer key, Pointer value) {
	bool removed;
	Pointer old_value = NULL;
	Pointer old_key= NULL;
	set->root = node_remove(set->root, set->compare, key, value, &removed, &old_key, &old_value);
...
Pointer set_find(Set set, Pointer key) {
	SetNode node = node_find_equal(set->root, set->compare, key);
...
DestroyFunc set_set_destroy_key(Set vec, DestroyFunc destroy_key) {
	DestroyFunc old = vec->destroy_key;
	vec->destroy_value = destroy_key;
	return old;
}

void set_destroy(Set set) {
	node_destroy(set->root, set->destroy_key, set->destroy_value);
	free(set);
}
...
Pointer set_node_overload_value(SetNode node) {
	return node->value;
}

Pointer set_node_key(Set set, SetNode node) {
	return node->key;
}

Pointer set_node_overload_key(SetNode node) {
	return node->key;
}

SetNode set_find_node(Set set, Pointer key) {
	return node_find_equal(set->root, set->compare, key);
}
...
bool node_is_avl(SetNode node, CompareFunc compare) {
	...
	bool res = true;
	if(node->left != NULL)
		res = res && compare(node->left->key, node->key) < 0 && compare(node_find_max(node->left)->key, node->key) < 0;
	if(node->right != NULL)
		res = res && compare(node->right->key, node->key) > 0 && compare(node_find_min(node->right)->key, node->key) > 0;
