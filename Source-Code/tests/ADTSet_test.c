...
void test_get_set_at(void) {
	// Η συνάρτηση αυτή ελέγχει ότι ένας κόμβος περιέχει πάντα την τιμή που
	// βρισκεται στη θεση position και οι συναρτησεις random access δουλευουν
	// σωστα και προσαρμοζονται και αναλογα αν καποιος κομβος προστιθεται ή
	// διαγραφεται και με την χρηση set_at αλλαζει τιμη
	...
	// αρχικα εξεταζω με την get_at αν εχουμε προσβαση στα
	// σωστα στοιχεια
	for (int i = 0; i < 1000; i++) {
		TEST_ASSERT(*(int*)set_get_at(set, i) == i);
	}

	// εδω εξεταζω αν αλλαξουν 5 στοιχεια 
	// με 5 καινουρια
	for (int i=0;i<5;i++)
		set_set_at(set, 0, create_int(2300+i));
	// αν το μεγεθος παραμεινει το ιδιο
	TEST_ASSERT(set_size(set)==1000);
	// και οτι εχει μπει σωστα στη τελευταια θεση το μεγαλυτερο
	TEST_ASSERT(*(int*)set_get_at(set, 999)==2304);


	// Εδω ελεγχω αν θεσω 5 στοιχεια στην θεση 0 με την ιδια τιμη
	// το 30 αν θα αντικατασταθουν στις σωστες θεσεις 
	for (int i=0;i<5;i++)
		set_set_at(set, 0, create_int(30));

	// οταν γινεται αυτο οπως λεει και η εκφωνηση στην ουσια
	// απαλειφονται τα προηγουμενα οποτε το size θα χει μειωθει
	// κατα 5 τωρα
	TEST_ASSERT(set_size(set)==995);
	// και το τελευταιο στοιχειο θα ναι μετατοπισμενο
	// οποτε εδω ελεγχω οτι η διαταξη δεν εχει αλλαξει
	// με αυτες τις αλλαγες
	// και εχω ακομα προσβαση με την get at 
	// στο σωστο στοιχειο
	TEST_ASSERT(*(int*)set_get_at(set, 0)==10);
	TEST_ASSERT(*(int*)set_get_at(set, 200)==210);
	TEST_ASSERT(*(int*)set_get_at(set, 989)==999);
	TEST_ASSERT(*(int*)set_get_at(set, 990)==2300);
	TEST_ASSERT(*(int*)set_get_at(set, 994)==2304);
...
TEST_LIST = {
	...
	{ "set_get_set_at",	test_get_set_at },
